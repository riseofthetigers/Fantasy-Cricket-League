...
if(isset($player_id) && !empty($player_id)) {
	...
	$result=mysqli_query($con,"select tc2,rc2,teamcomplete from users where user_id='$user_id'");
	while($row=mysqli_fetch_array($result)) {
		$tc=$row['tc2'];
		$rc=$row['rc2'];
	...
	for($i=0;$i<$nteams;$i++) {
		...
		if($teamCount[$i]==14 && $team==$teams[$i])
		$count++;
	}
	if($count==1)
		$f=1;
	...
	if($flag==1)
	{
			...
			if($tc>=1 && $rc<$credits) {
				...
				if($tc!=0) {
					if($f==1) {
						...
						if($tc==2 && $skill!='wk' && $wk==0) {
							echo 'No Wicket-Keeper!';
						}
						else if($tc==2 && $skill!='ar' && $ar==0) {
							echo 'No All-Rounder!';
						}
						else if($tc==4 && $skill!='ar' && $ar==0 && $skill!='wk' && $wk==0) {
							echo 'No Wicket-Keeper and no All-Rounder!';
						}
						else if($tc==6 && $skill!='bowler' && $bowler==0) {
							echo 'No bowler!';
						}
						else if($tc==6 && $skill!='batsman' && $batsman==0) {
							echo 'No batsman!';
						}
						else {
							switch($skill) {
								case 'wk':
									if($wk==2) {
										...
										mysqli_query($con,"update users set rc2='$rc',tc2='$tc' where user_id='$user_id'");
		...
		if($alreadyThere==1) {
			...
			mysqli_query($con,"update users set teamcomplete=0,rc2='$rc',tc2='$tc' where user_id='$user_id'");
